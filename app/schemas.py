...
class ClientFeatures(BaseModel):
    ...
    sexo: str = Field(..., regex="^(V|H|U|)$", description="Пол: V/H/U")
    segmento: str = Field(..., description="Сегмент клиента")
    ind_empleado: str = Field(..., regex="^(A|B|F|N|S|)$", description="Статус занятости")
...
class PredictionResponse(BaseModel):
    ...
    recommendations: List[str] = Field(..., max_items=7, description="Топ-7 рекомендуемых продуктов")
